...
public class LanderBracketDetector extends DogeCVDetector {

    public double bestDiffrence = Double.MAX_VALUE;
    ...
    private double  goldYPos = 0;     // ****CORE WROTE THIS****
    ...
    public double blindYValue     = 0;    // ****CORE WROTE THIS****

    public DogeCV.AreaScoringMethod areaScoringMethod = DogeCV.AreaScoringMethod.MAX_AREA; // Setting to decide to use MaxAreaScorer or PerfectAreaScorer

    public double currRatio = 0;

    //Create the default filters and scorers
    public DogeCVColorFilter yellowFilter      = new LeviColorFilter(LeviColorFilter.ColorPreset.WHITE); //Default Yellow filter

    public RatioScorer ratioScorer       = new RatioScorer(1.0, 1);          // Used to find perfect squares
    public MaxAreaScorer maxAreaScorer     = new MaxAreaScorer( 1);                    // Used to find largest objects
    public PerfectAreaScorer perfectAreaScorer = new PerfectAreaScorer(5000,0); // Used to find objects near a tuned area value

    /**
     * Simple constructor
     */
    public LanderBracketDetector() {
        super();
    ...
    public Mat process(Mat input) {
        ...
        Imgproc.line(displayMat, new Point(0, blindYValue), new Point(630,blindYValue), new Scalar(0,0,255));

        if(bestRect != null && bestRect.y + (bestRect.height / 2) > blindYValue) {
            currRatio = bestRect.height / bestRect.width;
    ...
    public void blindCamera(double blindVal) {blindYValue = blindVal;}
